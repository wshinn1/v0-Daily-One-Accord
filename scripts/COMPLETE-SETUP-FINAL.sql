...
-- Simple policies for other tables (tenant-scoped)
CREATE POLICY "Users can manage their church data" ON church_members FOR ALL
...
CREATE POLICY "Users can manage data" ON custom_roles FOR ALL
  USING (church_tenant_id IN (SELECT church_tenant_id FROM users WHERE id = auth.uid()));

CREATE POLICY "Users can manage data" ON event_registrations FOR ALL
  USING (event_id IN (SELECT id FROM events WHERE church_tenant_id IN (SELECT church_tenant_id FROM users WHERE id = auth.uid())));

CREATE POLICY "Users can manage data" ON ministry_team_members FOR ALL
  USING (ministry_team_id IN (SELECT id FROM ministry_teams WHERE church_tenant_id IN (SELECT church_tenant_id FROM users WHERE id = auth.uid())));

CREATE POLICY "Users can manage data" ON volunteer_team_members FOR ALL
  USING (volunteer_team_id IN (SELECT id FROM volunteer_teams WHERE church_tenant_id IN (SELECT church_tenant_id FROM users WHERE id = auth.uid())));

CREATE POLICY "Users can manage data" ON newsletter_recipients FOR ALL
  USING (newsletter_id IN (SELECT id FROM newsletters WHERE church_tenant_id IN (SELECT church_tenant_id FROM users WHERE id = auth.uid())));

CREATE POLICY "Users can manage data" ON rundown_modules FOR ALL
  USING (rundown_id IN (SELECT id FROM event_rundowns WHERE church_tenant_id IN (SELECT church_tenant_id FROM users WHERE id = auth.uid())));
